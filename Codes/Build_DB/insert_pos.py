...
def insert_forward(c, table_name, data):
    cmd = "INSERT OR IGNORE INTO " + table_name + " (S_Header, F_start_POS) VALUES(?,?);"
    cmd2 = "UPDATE " + table_name + " SET F_start_POS = ? WHERE S_Header = ?;"
    c.execute(cmd, data)
    data2 = (data[1],data[0])
    c.execute(cmd2, data2)

def insert_reverse(c, table_name, data):
    cmd = "INSERT OR IGNORE INTO " + table_name + " (S_Header, R_start_POS) VALUES(?,?);"
    cmd2 = "UPDATE " + table_name + " SET R_start_POS = ? WHERE S_Header = ?;"
    c.execute(cmd, data)
    data2 = (data[1],data[0])
    c.execute(cmd2, data2)
